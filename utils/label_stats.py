...
from dataset import discover_clip_items
...
def aggregate_frame_counts_from_json(
    ...
    items = discover_clip_items(root, split, vdir, ldir, stem_to_rel)
...
def count_events_per_class_from_json(
    ...
    stem_to_rel = load_stem_to_relpath(root)
    items = discover_clip_items(root, split, root / video_dir, root / labels_dir, stem_to_rel)
